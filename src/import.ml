...
external add_function_import : Module.t -> string -> string -> string -> Type.t -> Type.t -> unit =
...
external add_table_import : Module.t -> string -> string -> string -> unit = "caml_binaryen_add_table_import"

(** Module, internal name, external module name, external base name, shared. *)
external add_memory_import : Module.t -> string -> string -> string -> bool -> unit = "caml_binaryen_add_memory_import"

(** Module, internal name, external module name, external base name, type, mutable. *)
external add_global_import : Module.t -> string -> string -> string -> Type.t -> bool -> unit = 
